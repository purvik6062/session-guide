...
function ensure_docker() {
  ...
  if docker images -q "$IMG" &> /dev/null | grep -q .; then
    echo "✅ Docker image '$IMG' already pulled"
  else
    say "📦 Pulling Docker image with platform flag '$IMG'..."
...
# Open new terminal with the push script (macOS specific)
if command -v osascript &> /dev/null; then
    osascript -e "tell app \"Terminal\" to do script \"$push_script_path\""
else
...
EOF
    ...
    # Open new terminal with the verify push script (macOS specific)
    if command -v osascript &> /dev/null; then
        osascript -e "tell app \"Terminal\" to do script \"$verify_push_script_path\""
    else
        echo "🔧 Please run this script manually in a new terminal:"
...
if [[ "$configure_smart_cache" =~ ^[Yy]$ ]]; then
  ...
  # Create update script (macOS compatible)
...
# Update deployed_by (macOS sed syntax)
...
# Update contract_address if provided (macOS sed syntax)
if [ -n "$2" ]; then
...
EOF
    ...
    # Open new terminal with the push script (macOS specific)
    if command -v osascript &> /dev/null; then
        osascript -e "tell app \"Terminal\" to do script \"$smart_cache_push_script_path\""
    else
        echo "🔧 Please run this script manually in a new terminal:"
...
if [[ "$configure_rust_crate" =~ ^[Yy]$ ]]; then
  ...
  if [[ -z "$cargo_dir" ]]; then
    ...
    if [[ -z "$lib_rs_path" ]]; then
      ...
      # Update lib.rs with import and cacheable function (macOS compatible sed)
...
if ! grep -q "use stylus_cache_sdk" "$libfile"; then
  # Add after extern crate alloc line (macOS sed syntax)
  sed -i "" "/extern crate alloc;/a\\
use stylus_cache_sdk::{is_contract_cacheable};" "$libfile"
fi
...
EOF
        ...
        # Open new terminal with the push script (macOS specific)
        if command -v osascript &> /dev/null; then
            osascript -e "tell app \"Terminal\" to do script \"$rust_push_script_path\""
        else
            echo "🔧 Please run this script manually in a new terminal:"
