...
function find_packages_nextjs() {
  currentDir=$(pwd)
  
  # Search in current directory
  packagesNextjsPath="$currentDir/packages/nextjs"
  if [ -d "$packagesNextjsPath" ]; then
    echo "$packagesNextjsPath"
    return 0
  fi
  ...
  for subDir in "$currentDir"/*/; do
    if [ -d "$subDir" ]; then
      subPackagesPath="$subDir/packages/nextjs"
      if [ -d "$subPackagesPath" ]; then
        echo "$subPackagesPath"
        return 0
      fi
    fi
  done
...
if ! packagesNextjsPath=$(find_packages_nextjs); then
